...
for epi = 1, epoch do
	...
	for line in io.lines(trainAnnotationPath) do
		...
		if (i == batchSize) then
			...
			local input = {} 
			--local targets_hard = {}
			local targets_hard = labelbatch:cuda()
			...
			input_tensor = nn.JoinTable(1):forward(input):reshape(bactchseqLenMax,batchSize,featdim):cuda()
			forOneTimeStep = nil
			--labeltemp_hard = nil
			----RNN	   	    
			local feval = function(x_new)
				...
				local output = unpack(model:forward(input_tensor))
				...
				model:backward(input_tensor, gradOutputs_hard)
