...
function test (model_attenLSTM,infile_test)
	...
	for i = 1, batch_num do
		...
		local inputs = featbatch_trans:clone()
	        ...
	        local output =  unpack(model_attenLSTM:forward(inputs))
...
---------------train_input
local infile = "randseq_train.h5"   
local myFile = hdf5.open(infile, 'r')
local feats  = myFile:read('feature'):all() -- sample_num x shot_num(sequence) x feat_dim = 1000 x 15 x 20
local labels = myFile:read('label'):all() 
myFile:close()
local sample_num = feats:size(1)
...
for epoch = 1, epoch_num do
	...
	print("\nepoch"..epoch)
	print("===========Test==============")
	--test(model_attenLSTM,"randseq_test.h5") 
